...
export default function PayedModal({ grandTotal }) {
  ...
  const cartItems = keys.map((item) => ({
    name: cart[item].name,
    price: cart[item].price,
    quantity: cart[item].quantity,
    image: cart[item].mobile.slice(1),
  }));
  ...
  // Cart elements for modal display
  const cartElements = keys.map((item, index) => (
    <div
      className="flex w-full justify-between items-center border-b border-zinc-200"
      key={`payModal${index}`}
    >
      ...
      <p className="opacity-50 font-bold text-sm self-start mt-3">
      ...
      <div className="w-full rounded-lg border bg-white max-w-xl py-8 px-7 z-50 mx-auto my-auto">
        ...
        <div className="my-6 sm:flex">
          <div className="bg-zinc-100 py-6 px-8 rounded-t-lg flex flex-col items-center sm:rounded-none sm:rounded-l-lg grow">
            ...
            {keys.length > 1 && (
              <p
                className="mt-3 opacity-50 cursor-pointer font-bold"
        ...
        <button
          ...
          className="bg-orange-500 text-white px-6 py-3 w-full mt-4"
        >
          Continue & Pay
        </button>
        ...
        <Link to={"/"} className="mt-4">
          <SeeProductBtn full={true} action={() => setCart({})}>
            back to home
          </SeeProductBtn>
        </Link>
      </div>
